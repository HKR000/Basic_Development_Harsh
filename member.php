...
if(checkUserSession($db) !== TRUE){
	header("location: $_LOGIN_FILE");exit; 
}
                    ...
                    <div class="ibox float-e-margins">
						...
						?>
								...
								<div class="social-avatar">
									<small class="text-muted">Joined at <?= $req["join_date"] ?></small>
								</div>
								<div class="social-body">
									<strong><?= $_user["firstName"] . " " . $_user["lastName"]; ?></strong> currently a member of the group.
								<p></p><hr><p></p>
								<div class="file-option">
									<!-- <button class="btn btn-primary btn-rounded btn-sm" onclick="approve_request(<?= $req["user_id"] ?>, <?=  $req["room_id"] ?>)"><i class="fa fa-check"></i> Submit to owner </button> -->
...
function approve_request(user_id, room_id){
	$.ajax({
		...
		success: function(r) {
			if(r.success){
				$("#request-user-" + user_id).remove()
				toastr.success("This member has been successfully approved.")
			} else {
				toastr.error("This member cannot be approved.")
			}
		},
		error: function(){
